...
import com.englishlearning.android.models.ListviewItem;
import com.englishlearning.android.service.WordService;
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        switch (item.getItemId()) {
            case R.id.action_cart://监听菜单按钮
                ...
                Cursor cursor = app.getDb().query("Book",new String[]{"wordNum"},"bookId=?",new String[]{wDao.getHelper().WORDLIST_TABLE_TMPWORD},null,null,null);
                ContentValues values1=new ContentValues();
                cursor.moveToFirst();
                values1.put("wordNum",cursor.getInt(cursor.getColumnIndex("wordNum"))+mData.size());
                app.getDb().update("Book",values1,"bookId=?",new String[]{wDao.getHelper().WORDLIST_TABLE_TMPWORD});
                Intent intent=new Intent();
